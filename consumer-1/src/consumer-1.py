...
def process_and_upload_to_s3(message):
    ...
    timestamp_epoch = message.get('END_AT', None)
    
    if timestamp_epoch:
        # Convert epoch to datetime
        timestamp = datetime.fromtimestamp(int(timestamp_epoch) / 1000.0)
        ...
        # Create file name (e.g., 01.avro)
        file_name = f"Aggrgated_Page_View_{hour}-{minute}.json"
...
def consume():
    ...
    while True:
        ...
        process_and_upload_to_s3(msg.value()["DATA"])
        msg_count += 1
        if msg_count % 2 == 0:
